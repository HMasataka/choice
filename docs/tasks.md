...
- [x] シグナリングプロトコル統合テスト基盤（MockConnectionHandler使用）
- [x] 2者間通話シグナリングフローテスト
- [x] 多人数会議シグナリングテスト（5人）
- [x] 途中参加・途中退出シナリオテスト
- [x] ネットワーク切断復旧シナリオテスト（セッションストア連携）
- [x] 並行操作ストレステスト（10クライアント同時）
- [x] メディアパブリッシュ/サブスクライブシグナリングフロー
- [ ] Playwright/Puppeteerによるブラウザ自動化環境構築（将来対応）
- [ ] クロスブラウザテスト（Chrome/Firefox/Safari）（将来対応）

> **注記**: 現在のE2EテストはMockConnectionHandlerを使用したシグナリングプロトコル統合テスト。
> 完全なWebRTCスタック（STUN/TURN/ICE）を含むブラウザ自動化テストは将来対応。

**コミットメッセージ**: `feat(test): add E2E, load, and security test infrastructure`

#### Task T4.2: 負荷テストの実装 ✅

- [x] WebSocket負荷テストクライアント実装（Go + gorilla/websocket）
- [x] 同時接続数テスト（100接続/ルーム）
- [x] ルーム数スケールテスト（50ルーム同時稼働）
- [x] メッセージスループットテスト
- [x] 長時間稼働サンプルテスト（30秒）
- [x] REST API負荷テスト（1000リクエスト）
- [x] ベンチマーク（接続確立、メッセージラウンドトリップ、ルーム作成）
- [ ] k6によるREST API負荷テスト環境構築（将来対応）
- [ ] WebRTC負荷テストクライアント実装（Go + pion）（将来対応）
- [ ] メディアスループットテスト（1Gbps）（将来対応）
- [ ] 長時間稼働テスト（24時間連続）（将来対応）

> **注記**: 現在の負荷テストはシグナリングメッセージ負荷をテスト。
> 実WebRTCメディア負荷テストは専用環境で別途実行する想定。
...
- [x] 認証バイパス試行テスト
- [x] 権限昇格試行テスト（subscriber/publisher/moderator/admin）
- [x] トークン改ざんテスト（alg:none攻撃、ペイロード改変、署名切り詰め）
- [x] 入力バリデーションテスト（SQLインジェクション、パストラバーサル、ヌルバイト）
- [x] JSONインジェクションテスト（プロトタイプ汚染、深いネスト、オーバーサイズ）
- [x] WebSocket DoSテスト（急速接続、メッセージフラッド、大容量メッセージ）
- [x] 不正SDPインジェクションテスト
- [x] パストラバーサルテスト
- [x] HTTPヘッダーインジェクションテスト
- [x] トークン有効期限/将来時刻/ルームID不一致検証
- [x] 既存ルームでの認証挙動確認テスト
- [x] Content-Type検証テスト

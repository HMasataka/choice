...
### 9.1 同一種別の複数トラック購読時のマッチング

現在のSDKバージョンでは、同一参加者から同じ種類のトラック（例：2つのビデオトラック）を同時に購読する場合、受信トラックと購読リクエストのマッチングが非決定的になる可能性があります。

**原因**: サーバーの`trackSubscribed`通知にmid（メディア識別子）が含まれていないため、WebRTCの`ontrack`イベントで受信したトラックを特定の購読リクエストに紐付けるための確実なキーがありません。

**影響を受けるケース**:
- 1人の参加者が同じ種類のトラックを複数公開している場合（例：画面共有＋カメラ映像）
- それらを同時に購読した場合、トラックが入れ替わる可能性がある

**推奨される回避策**:
- 同一種類の複数トラックを購読する場合は、順次購読する（1つ目の購読完了を待ってから2つ目を購読）
- または、トラックのmetadataやnameを活用して、購読後にアプリケーション側で識別する

**今後の改善予定**:
- サーバー側で`trackSubscribed`通知にmid情報を追加予定
- SDK側でmidベースの厳密マッチングを実装予定

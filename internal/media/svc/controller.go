...
// Controller manages SVC layer selection for subscribers.
// Per design.md section 4.1: SVC support for VP9/AV1.
type Controller interface {
	// SetPreferredLayer sets the subscriber's preferred SVC layer.
	// This is called when a client explicitly requests a layer.
	SetPreferredLayer(ctx context.Context, subscriptionID media.SubscriptionID, layer SVCLayer) error

	// GetCurrentLayer returns the actual layer being sent.
	GetCurrentLayer(subscriptionID media.SubscriptionID) (SVCLayer, error)

	// GetPreferredLayer returns the client's preferred layer (may differ from actual).
	GetPreferredLayer(subscriptionID media.SubscriptionID) (SVCLayer, error)

	// OnBandwidthEstimate handles bandwidth updates from TWCC/REMB.
	// Returns layer change results if any actual layers changed.
...
// SubscriptionState holds the state for a single SVC subscription.
...
func (s *SubscriptionState) Copy() *SubscriptionState {
	...
	layers := make([]SVCLayer, len(s.AvailableLayers))
...
// LayerChangeResult contains information about an SVC layer change.
type LayerChangeResult struct {
	SubscriptionID media.SubscriptionID
	Changed        bool
	PreviousLayer  SVCLayer
	CurrentLayer   SVCLayer
...
// ControllerConfig contains configuration for the SVC controller.
...
// NewController creates a new SVC controller.
...
// SetPreferredLayer sets the subscriber's preferred SVC layer.
func (c *controller) SetPreferredLayer(ctx context.Context, subscriptionID media.SubscriptionID, layer SVCLayer) error {
	...
	if !layersEqual(previousLayer, actualLayer) && c.onLayerChange != nil {
		reason := LayerChangeReasonRequested
		if !layersEqual(actualLayer, layer) {
...
// GetCurrentLayer returns the actual layer being sent.
func (c *controller) GetCurrentLayer(subscriptionID media.SubscriptionID) (SVCLayer, error) {
	if err := subscriptionID.Validate(); err != nil {
		return SVCLayer{}, fmt.Errorf("invalid subscription ID: %w", err)
	}
	...
	state, exists := c.subscriptions[subscriptionID]
	if !exists {
		return SVCLayer{}, fmt.Errorf("subscription %s not found", subscriptionID)
	}
...
func (c *controller) GetPreferredLayer(subscriptionID media.SubscriptionID) (SVCLayer, error) {
	if err := subscriptionID.Validate(); err != nil {
		return SVCLayer{}, fmt.Errorf("invalid subscription ID: %w", err)
	}
...
func (c *controller) OnBandwidthEstimate(ctx context.Context, subscriberID string, bps uint64) []LayerChangeResult {
	...
	for subID := range subIDs {
		...
		if !layersEqual(newLayer, previousLayer) {
...
func (c *controller) OnPacketLoss(ctx context.Context, subscriberID string, lossRate float64) []LayerChangeResult {
	...
	for subID := range subIDs {
		...
		previousLayer := state.ActualLayer
		newLayer := c.layerSelector.SelectLayerForPacketLoss(
			state.AvailableLayers,
			state.PreferredLayer,
			state.ActualLayer,
			lossRate,
			bps,
		)
		...
		if !layersEqual(newLayer, previousLayer) {
			...
			if c.layerSelector.layerPriority(newLayer) > c.layerSelector.layerPriority(previousLayer) {
...
func (c *controller) RegisterSubscription(
	...
	layers := make([]SVCLayer, len(availableLayers))
...
// layersEqual compares two SVC layers for equality.
func layersEqual(a, b SVCLayer) bool {
	return a.SpatialLayer == b.SpatialLayer && a.TemporalLayer == b.TemporalLayer
}

...
// This is a simplified check - in production, parse the actual codec data
...
// isVP8Keyframe checks if VP8 payload is a keyframe
...
// isVP9Keyframe checks if VP9 payload is a keyframe
func isVP9Keyframe(payload []byte) bool {
	if len(payload) < 1 {
		return false
	}
	// VP9 keyframe detection (simplified)
	// Check P bit in the first byte
	return (payload[0] & 0x04) == 0
}

// isH264Keyframe checks if H264 payload is a keyframe
func isH264Keyframe(payload []byte) bool {
	if len(payload) < 2 {
		return false
	}
	// H264 NAL unit type 5 = IDR slice (keyframe)
